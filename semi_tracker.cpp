...
public:
    ...
    : it_(nh_)
  {
    ...
    pub = nh_.advertise<geometry_msgs::PointStamped>("/image_tracked/trackedpoint",1000);
  ...
  void imageCb(const sensor_msgs::ImageConstPtr& msg)
  {
    ...
    if(posX>=0 && posY>=0){
      ...
      cv::circle(cv_ptr->image, cv::Point(posX,posY),10,cv::Scalar(0,0,255),-1);
      cv::circle(imgThresh, cv::Point(posX,posY),10,cv::Scalar(0,0,255),-1);
...
cv::Mat GetThresholdedImage(cv::Mat imgHSV){
  ...
  //blue
  cv::inRange(imgHSV,cv::Scalar(90,100,130),cv::Scalar(140,256,256),imgThresh);
  //red
